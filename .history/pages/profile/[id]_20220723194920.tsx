...
const Profile = ({ data }: IProps) => {
  ...
  const { user, useVideos, userLikdedVideos } = data;
    ...
    <div className="w-full">
      ...
      <div>
        <div className="flex gap-10 mb-10 mt-10 border-b-2 border-gray-200 bg-white w-full">
         <p className={`text-xl font-semibold cursor-pointer mt-2 ${videos}`} onClick={serShowUserVideos(TextTrackCue)}>Videos</p>
        </div>
      </div>
    </div>
  );
